...
<%
	...
	int startRow = (currentPage-1) * pageSize +1 ; // 1 6 11 ... // db에 입력하면 그 사이값을 불러올것이다 > 결정되는 것은 페이지 수에 따라.
...
if(count>0){ // 게시글 38개, 한 화면 5개 ==> 38/5+38%5 ==0? 0:1 = 7개 + 1개 ==> 8개
	...
	int pageBlock = 3; // 페이징 3개 숫자
	int startPage = (int)((currentPage-1)/pageBlock) * pageBlock +1; // (7-1)/3 ==> 1*3+1 ==> 7 
	int endPage = startPage + pageBlock -1; // 7+3-1 ==> 9
	if(endPage > pageCount){ // 9(계산상 페이지) > 8(실제 페이지 수)
